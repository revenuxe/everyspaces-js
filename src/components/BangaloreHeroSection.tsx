...
function BangaloreHeroSectionInner() {
  ...
  const locationLabel = area ? `${area}, Bangalore` : "Bangalore";
  const sourcePage = area ? `/bangalore?area=${encodeURIComponent(area)}` : "/bangalore";
...
const BangaloreHeroSection = () => {
  return (
    <Suspense fallback={<BangaloreHeroSectionInnerFallback />}>
      <BangaloreHeroSectionInner />
    </Suspense>
  );
};

function BangaloreHeroSectionInnerFallback() {
  const router = useRouter();
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState({
  ...
  const locationLabel = "Bangalore";
  const sourcePage = "/bangalore";
