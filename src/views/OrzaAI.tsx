    ...
    <div className="h-[100dvh] bg-white flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 bg-white border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          <button onClick={() => stepHistory.length > 0 ? handleGoBack() : router.back()} className="w-9 h-9 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 pt-4 pb-4 space-y-3">
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              {msg.role === "assistant" && (
                <img src={imgSrc(orzaIcon)} alt="Orza" className="w-6 h-6 rounded-full mr-2 mt-1 shrink-0 bg-white object-contain" />
              )}
              <div
                ...
                <div className="flex gap-2 flex-wrap">
                  {uploadedImages.map((img, i) => (
                    <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-secondary/30">
                      <img src={imgSrc(img)} alt={`Upload ${i + 1}`} className="w-full h-full object-cover" />
